...
const INCC_CACHE_KEY  = 'incc_cache_v6';
...
const INCC_SERIE = [
  ...
  { m: 'Mai/26', v: 1270.695, var: 0.88, varAno: 3.46, var12m: 6.66 },
];
...
const INCCChart = ({ serie }) => {
  ...
  return (
    <svg className="chart-svg" viewBox={`0 0 ${w} ${h}`} preserveAspectRatio="xMidYMid meet">
      ...
      {serie.map((d, i) => {
        ...
        return (
          <g key={i} style={{ cursor: 'pointer' }} onClick={() => setSel(sel === i ? null : i)}>
            <rect
              x={xOf(i) - barW / 2} y={y}
              width={barW} height={pad.t + innerH - y}
              rx="3" fill="url(#bar-grad)"
              fillOpacity={isSelected ? 1 : barOpacity(d.var)}
            />
            {isSelected && (
              <>
                <rect x={xOf(i) - 24} y={y - 22} width={48} height={18} rx="4" fill="#014386" />
      ...
      <line x1={pad.l} x2={w - pad.r} y1={yOf(mediaVar)} y2={yOf(mediaVar)}
        stroke="var(--danger)" strokeWidth="1.4" strokeDasharray="5 4" />
